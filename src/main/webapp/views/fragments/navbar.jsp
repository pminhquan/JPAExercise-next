...
<c:choose>
    ...
    <c:when test="${currentServletPath == '/views/home.jsp' or currentServletPath == '/home' or currentServletPath == '/index.jsp'}">
    ...
    <c:when test="${currentServletPath == '/products' or currentServletPath == '/products/add' or currentServletPath == '/products/edit' or currentServletPath == '/views/product-add.jsp' or currentServletPath == '/views/product-edit.jsp'}">
        <c:set var="activeNav" value="products" />
    </c:when>
    <c:when test="${currentServletPath == '/categories' or currentServletPath == '/views/category-list.jsp' or currentServletPath == '/views/category-add.jsp' or currentServletPath == '/views/category-edit.jsp'}">
        <c:set var="activeNav" value="categories" />
    </c:when>
    <c:when test="${currentServletPath == '/views/product-detail.jsp' or currentServletPath == '/products/detail'}">
        <c:choose>
            <c:when test="${param.from == 'manage' or param.from == 'products' or (not empty sessionScope.authenticatedUserId and not empty param.from and param.from != 'catalog' and param.from != 'product')}">
                <c:set var="activeNav" value="products" />
            </c:when>
            <c:otherwise>
                <c:set var="activeNav" value="catalog" />
    ...
    <c:when test="${currentServletPath == '/views/login.jsp' or currentServletPath == '/login'}">
        <c:set var="activeNav" value="login" />
    </c:when>
    <c:when test="${currentServletPath == '/views/register.jsp' or currentServletPath == '/register'}">
...
<header class="site-header">
    <nav class="site-nav navbar navbar-expand-lg" aria-label="Main navigation">
        <div class="container">
            <a class="navbar-brand site-brand" href="${pageContext.request.contextPath}/home" aria-label="JPAExercise Home">
                <span class="site-brand__mark" aria-hidden="true">J</span>
                <span class="site-brand__text">JPAExercise</span>
            </a>
